...
async function main() {
  ...
  for (let index = 0; index < args.length; index++) {
    ...
    else if (arg === '--input' && !inputSeen && args[index + 1] && !args[index + 1]!.startsWith('--')) {
      input = args[++index]!;
      inputSeen = true;
  ...
  catch { throw new Error('Fixture file is not valid JSON.'); }
  const report = evaluate(parseCases(parsed));
  if (json) console.log(JSON.stringify(report, null, 2));
  else {
    console.log('INTAKE EVAL — synthetic regression suite\n');
    for (const result of report.results) console.log(`${result.passed ? 'PASS' : 'FAIL'}  ${result.id} → ${result.decision}${result.queue ? ` / ${result.queue}` : ''}`);
...
main().catch((error: unknown) => {
  ...
  console.error(code ? `Could not read fixture file (${String(code)}).` : error instanceof Error ? error.message : 'Evaluation failed.');
  process.exitCode = 2;
});
