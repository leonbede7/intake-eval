...
test('accepts raw JSON and returns the same result as the parsed object', () => {
  assert.deepEqual(
    reviewCandidate(source, JSON.stringify(valid())),
    reviewCandidate(source, valid()),
  );
});
...
test('rejects invented, case-changed and whitespace-changed quotes', () => {
  for (const quote of [
    ...
    'the export button gives an error',
    'The export  button gives an error',
  ]) {
    assert.equal(
      reviewCandidate(source, { ...valid(), evidence: [quote] }).issues[0]?.code,
      'UNGROUNDED_EVIDENCE',
    );
  }
});
test('an empty, too-short, duplicate or non-string quote cannot count as evidence', () => {
  for (const evidence of [[], [''], ['The'], [42], [valid().evidence[0], valid().evidence[0]]]) {
    assert.equal(
      reviewCandidate(source, { ...valid(), evidence }).issues[0]?.code,
      'INVALID_SCHEMA',
    );
  }
});
...
test('rejects oversized raw candidate and summary', () => {
  ...
  assert.equal(
    reviewCandidate(source, { ...valid(), summary: 'a'.repeat(281) }).issues[0]?.code,
    'INVALID_SCHEMA',
  );
});
...
test('known boundary: exact evidence does not prove the summary is true', () => {
  const result = reviewCandidate(source, {
    ...valid(),
    summary: 'The customer has successfully exported everything.',
  });
  assert.equal(result.decision, 'review_required');
...
test('returns an independent candidate snapshot', () => {
  ...
  if (result.decision === 'review_required')
    assert.equal(result.candidate.evidence[0], 'The export button gives an error');
