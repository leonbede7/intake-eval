...
const fixtures: unknown = JSON.parse(
  readFileSync(new URL('../fixtures/triage-cases.json', import.meta.url), 'utf8'),
);
...
test('fixture validation rejects missing candidate, queue and misspelled expectation keys', () => {
  ...
  assert.throws(() =>
    parseCases([{ ...first, expected: { decision: 'rejected', issueCode: 'INVALID_JSON' } }]),
  );
});
test('wrong decision, queue or issue codes are regressions', () => {
  ...
  assert.equal(
    evaluate([{ ...cases[0]!, expected: { decision: 'review_required', queue: 'escalation' } }])
      .failed,
    1,
  );
  assert.equal(
    evaluate([{ ...cases[6]!, expected: { decision: 'rejected', issueCodes: ['INVALID_SCHEMA'] } }])
      .failed,
    1,
  );
});
...
test('CLI returns exit 2 for bad arguments and missing files', () => {
  for (const args of [
    ...
    ['--input'],
    ['--json', '--json'],
    ['--input', 'nonexistent-fixture-file.json'],
  ]) {
...
test('CLI returns exit 1 for regression and exit 2 for malformed fixtures', () => {
  ...
  try {
    ...
    writeFileSync(
      file,
      JSON.stringify([{ ...parseCases(fixtures)[0], expected: { decision: 'rejected' } }]),
    );
